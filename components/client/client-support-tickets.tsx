...
import { LifeBuoy, Plus, Upload, Download, Home, ChevronRight } from "lucide-react"
import { SummaryCards } from "./support-tickets/components/SummaryCards"
import { SearchAndFilters } from "./support-tickets/components/SearchAndFilters"
import { TicketTableView } from "./support-tickets/components/TicketTableView"
import { TicketCardView } from "./support-tickets/components/TicketCardView"
import { PaginationControls } from "./support-tickets/components/PaginationControls"
import { AdvancedFilterSheet } from "./support-tickets/components/AdvancedFilterSheet"
import AddTicketDrawer from "./AddTicketDrawer"
import { Badge } from "@/components/ui/badge"

const mockTickets = [
  { id: "101", subject: "Unable to Access Security Reports", category: "access", priority: "high", status: "in_progress", recipient: "Manager", description: "403 while downloading reports", updatedAt: "2025-01-15T10:00:00Z", comments: 4 },
  { id: "102", subject: "Clarification on Network Configuration", category: "request", priority: "medium", status: "open", recipient: "Admin", description: "Need firewall rules guidance", updatedAt: "2025-01-14T12:00:00Z", comments: 2 },
  { id: "103", subject: "Scheduling New Penetration Test", category: "request", priority: "low", status: "open", recipient: "Both", description: "Schedule Q1 2025 test", updatedAt: "2025-01-12T08:00:00Z", comments: 1 },
  { id: "104", subject: "Billing discrepancy", category: "billing", priority: "critical", status: "resolved", recipient: "Admin", description: "Invoice amount mismatch", updatedAt: "2025-01-10T08:00:00Z", comments: 3 },
]
...
export default function ClientSupportTickets({ onBack, onNavigate }: ClientSupportTicketsProps) {
  ...
  const handleAddTicket = (data: any) => {
    const newTicket = { id: String(100 + tickets.length + 1), updatedAt: new Date().toISOString(), comments: 0, status: "open", ...data }
  ...
  const handleImportButtonClick = () => {
    const fileInput = importFileInput || document.querySelector('input[type="file"][accept=".json"][data-scope="tickets"]') as HTMLInputElement
  ...
  const handleViewTicket = (t: any) => { 
    if (onNavigate) {
      onNavigate("add-ticket", { ticket: t, isViewMode: true })
    } else {
      setSelectedTicket(t); setEditingTicket(undefined); setShowAddDrawer(true) 
    }
  }
  const handleEditTicket = (t: any) => { 
    if (onNavigate) {
      onNavigate("add-ticket", { ticket: t, isEditMode: true })
    } else {
      setEditingTicket(t); setSelectedTicket(undefined); setShowAddDrawer(true) 
    }
  }
  ...
  const handleAddTicketClick = () => {
    if (onNavigate) {
      onNavigate("add-ticket")
    } else {
      setSelectedTicket(undefined); setEditingTicket(undefined); setShowAddDrawer(true)
    }
  }
          ...
          <div className="flex justify-between items-center h-full">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm">Manage and track your support requests</p>
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <Button size="sm" className="h-9 bg-gradient-to-r from-[#0d9488] to-[#0891b2] text-white shadow-md hover:from-[#0a7c71] hover:to-[#0784a0]" onClick={handleAddTicketClick}>
