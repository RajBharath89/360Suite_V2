...
const ticketFormSchema = z.object({
  ...
  recipient: z.enum(["Admin", "Tester", "Both"]),
...
export default function ManagerAddTicketPage({
  onBack,
  onSave,
  defaultClient,
  availableServices,
  ...
  const [messages, setMessages] = useState<Array<{ author: "Admin" | "Manager"; text: string; timestamp: string }>>([])
  ...
  useEffect(() => {
    if (editingTicket) {
      ...
      const initialMessages = (editingTicket.messages as Array<{ author: "Admin" | "Manager"; text: string; timestamp: string }>) || []
  ...
  const handleSendReply = () => {
    ...
    if (!trimmed) return
    const newMsg = { author: "Manager" as const, text: trimmed, timestamp: new Date().toISOString() }
        ...
        {isViewMode && (
          <Card className="h-fit">
            <CardHeader className="pb-3">
              ...
              <CardDescription>Conversation between Admin and Manager</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                {messages && messages.length > 0 ? (
                  messages.map((m, idx) => (
                    <div key={idx} className={`flex ${m.author === "Manager" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[80%] rounded-lg px-3 py-2 text-sm shadow ${m.author === "Manager" ? "bg-cyan-50 border border-cyan-100" : "bg-gray-50 border"}`}>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-xs font-medium ${m.author === "Manager" ? "text-cyan-700" : "text-gray-700"}`}>{m.author}</span>
