...
import { mockUsers } from "../data/mockWorkflowData"
...
interface TimelineViewProps {
  ...
  onReassign?: (testerId: string, reason?: string) => void
}
...
export function TimelineView({
  ...
  const [reassigningReportId, setReassigningReportId] = useState<string | null>(null)
                  ...
                  >
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-4 space-y-4">
                        ...
                        {/* Stage 10: Client Review - Approved and Rejected Reports */}
                        {stage.id === 10 && stage.statuses[0]?.approvalHistory && stage.statuses[0].approvalHistory.length > 0 && (
                          <div className="space-y-4">
                            {/* Approved Reports */}
                            {stage.statuses[0].approvalHistory.filter(a => a.action === 'approved').length > 0 && (
                              <div className="rounded-md border border-emerald-200 bg-emerald-50/60 p-3">
                                <div className="flex items-center gap-2 mb-3">
                                  <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                                  <h4 className="text-sm font-semibold text-emerald-900">
                                    Approved Reports ({stage.statuses[0].approvalHistory.filter(a => a.action === 'approved').length})
                                  </h4>
                                </div>
                                <div className="space-y-3">
                                  {stage.statuses[0].approvalHistory
                                    .filter(approval => approval.action === 'approved')
                                    .map((approval) => (
                                      <div key={approval.id} className="p-3 bg-white rounded-md border border-emerald-200">
                                        <div className="flex items-start justify-between mb-2">
                                          <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                              <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                                              <p className="text-sm font-semibold text-gray-900">{approval.reportName}</p>
                                              <Badge className="bg-emerald-100 text-emerald-800 border-emerald-200 text-xs">
                                                v{approval.reportVersion}
                                              </Badge>
                                            </div>
                                        ...
                                        {approval.reportUrl && (
                                          <Button
                                            ...
                                            size="sm"
                                            className="mt-2"
                                            onClick={() => window.open(approval.reportUrl, '_blank')}
                                          >
                            ...
                            {/* Rejected Reports */}
                            {stage.statuses[0].approvalHistory.filter(a => a.action === 'rejected').length > 0 && (
                              <div className="rounded-md border border-rose-200 bg-rose-50/60 p-3">
                                <div className="flex items-center gap-2 mb-3">
                                  <XCircle className="h-4 w-4 text-rose-600" />
                                  <h4 className="text-sm font-semibold text-rose-900">
                                    Rejected Reports ({stage.statuses[0].approvalHistory.filter(a => a.action === 'rejected').length})
                                  </h4>
                                </div>
                                <div className="space-y-3">
                                  {stage.statuses[0].approvalHistory
                                    ...
                                    .map((approval) => (
                                      <div key={approval.id} className="p-3 bg-white rounded-md border border-rose-200">
                                        <div className="flex items-start justify-between mb-2">
                                          <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                              <XCircle className="h-4 w-4 text-rose-600" />
                                            ...
                                            {approval.reason && (
                                              <div className="p-2 bg-rose-50 rounded-md mb-2">
                                        ...
                                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                                          ...
                                          <Dialog 
                                            open={showReassignDialog && reassigningReportId === approval.id} 
                                            onOpenChange={(open) => {
                                              setShowReassignDialog(open)
                                              if (!open) {
                                                setReassigningReportId(null)
                                                setSelectedTester("")
                                          ...
                                          >
                                            <DialogTrigger asChild>
                                              <Button
                                                variant="outline"
                                                size="sm"
                                                className="bg-blue-50 text-blue-700 hover:text-blue-800 hover:bg-blue-100 border-blue-300"
                                                onClick={() => {
                                                  setReassigningReportId(approval.id)
                                                  setShowReassignDialog(true)
                                                }}
                                              >
                                                <User className="h-3 w-3 mr-1" />
                                                Reassigned to Manager
                                              </Button>
                                            </DialogTrigger>
                                            <DialogContent>
                                              <DialogHeader>
                                                ...
                                                <DialogDescription>
                                                  Assign this service to a tester. The workflow will continue from Task Assignment (Stage 4).
                                                </DialogDescription>
                                              </DialogHeader>
                                              <div className="space-y-4 py-4">
                                                ...
                                                <div>
                                                  <label className="text-sm font-medium mb-2 block">Reassignment Notes (Optional)</label>
                                                  <Textarea
                                                    value={reassignmentReason}
                                                    onChange={(e) => setReassignmentReason(e.target.value)}
                                                    placeholder="Add notes about why this is being reassigned..."
                                              ...
                                              <DialogFooter>
                                                ...
                                                <Button 
                                                  onClick={() => {
                                                    if (selectedTester) {
                                                      if (onReassign) {
                                                        onReassign(selectedTester, reassignmentReason || approval.reason)
                                                      }
                                                      setShowReassignDialog(false)
                                                      setReassigningReportId(null)
                                                      setSelectedTester("")
                                                ...
                                                >
                                                  Reassign & Continue from Stage 4
                                                </Button>
                                              </DialogFooter>
                                            </DialogContent>
                                          </Dialog>
                        ...
                        {/* Stage 9: Manager Review - Approved and Rejected Reports */}
                        {stage.id === 9 && stage.statuses[0]?.approvalHistory && stage.statuses[0].approvalHistory.length > 0 && (
                          <div className="space-y-4">
                            {/* Approved Reports */}
                            {stage.statuses[0].approvalHistory.filter(a => a.action === 'approved').length > 0 && (
                              <div className="rounded-md border border-emerald-200 bg-emerald-50/60 p-3">
