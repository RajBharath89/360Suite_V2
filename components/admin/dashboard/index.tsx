...
import { DashboardDataProcessor } from "./data/dashboardData"
import { DashboardCharts } from "./components/DashboardCharts"
import { DashboardFilters } from "./components/DashboardFilters"
import { SummaryCards } from "./components/SummaryCards"
import { mockTimelines } from "../workflow/data/mockWorkflowData"
...
export default function DashboardPage() {
  ...
  const clients = useMemo(() => {
    const uniqueClients = new Set(mockTimelines.map(t => {
      const clientNames: Record<string, string> = {
        '1': 'TechCorp Solutions',
        '2': 'FinanceFirst Bank',
        '3': 'HealthCare Plus'
      }
      return clientNames[t.clientId] || `Client ${t.clientId}`
    }))
    return Array.from(uniqueClients)
  }, [])

  const serviceTypes = useMemo(() => {
    const uniqueServices = new Set(mockTimelines.map(t => t.serviceName))
  ...
  const filteredData = useMemo(() => {
    ...
    if (selectedClient !== "all") {
      ...
      const clientId = clientIdMap[selectedClient]
      if (clientId) {
    ...
    if (selectedServiceType !== "all") {
      filtered = filtered.filter(t => t.serviceName === selectedServiceType)
    }
  ...
  const mockTickets = useMemo(() => ([
    ...
    { id: "103", status: "on_hold" },
    { id: "104", status: "resolved" },
    { id: "105", status: "open" }
  ]), [])
      ...
      <Card className="bg-gradient-to-r from-cyan-50 to-green-50 border-cyan-200">
        <CardHeader>
          ...
          <CardDescription className="text-lg">
            Automated analysis of your workflow performance
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-600">
                {metrics.averageProgress}% average completion rate across all workflows
              </p>
            </div>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-gray-600">
                {metrics.activeWorkflows} active workflows with {metrics.completedWorkflows} completed
              </p>
            </div>
            <div className="space-y-2">
              <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-gray-600">
                {metrics.blockedWorkflows + metrics.overdueWorkflows} workflows need immediate attention
              </p>
            </div>
          </div>
        </CardContent>
