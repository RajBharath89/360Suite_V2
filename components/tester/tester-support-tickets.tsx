...
interface TesterSupportTicketsProps {
  onNavigate?: (page: string, data?: any) => void
  tickets?: any[]
  onTicketsChange?: (tickets: any[]) => void
}

export default function TesterSupportTickets({ onNavigate, tickets: externalTickets, onTicketsChange }: TesterSupportTicketsProps = {}) {
  const [internalTickets, setInternalTickets] = useState(mockTickets)
  const tickets = externalTickets ?? internalTickets
  const setTickets = (newTickets: any[] | ((prev: any[]) => any[])) => {
    if (typeof newTickets === 'function') {
      const updated = newTickets(tickets)
      if (onTicketsChange) onTicketsChange(updated)
      else setInternalTickets(updated)
    } else {
      if (onTicketsChange) onTicketsChange(newTickets)
      else setInternalTickets(newTickets)
    }
  }
  ...
  const handleExportTickets = () => {
    try {
      ...
      link.setAttribute('download', `tester_tickets_export_${new Date().toISOString().split('T')[0]}.json`)
  ...
  const handleImportButtonClick = () => {
    const fileInput = importFileInput || document.querySelector('input[type="file"][accept=\".json\"][data-scope="tester-tickets"]') as HTMLInputElement
  ...
  const wrappedHandleViewTicket = (t: any) => {
    if (onNavigate) {
      onNavigate("view-ticket", t)
    } else {
      handleViewTicket(t)
    }
  }

  const wrappedHandleEditTicket = (t: any) => {
    if (onNavigate) {
      onNavigate("edit-ticket", t)
    } else {
      handleEditTicket(t)
    }
  }
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <CardContent className="p-3 h-full">
          <div className="flex justify-between items-center h-full">
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" className="h-9 bg-gradient-to-r from-[#0d9488] to-[#0891b2] text-white shadow-md hover:from-[#0a7c71] hover:to-[#0784a0]" onClick={wrappedHandleAddTicket}>
      ...
      <input
        type="file"
        accept=".json"
        data-scope="tester-tickets"
        ref={(input) => setImportFileInput(input)}
        onChange={handleImportTickets}
        className="hidden"
        aria-label="Import tester tickets JSON file"
      />
